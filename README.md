...
- **Node.js 20+ / pnpm 9+**(ローカルビルド用。pnpm は Node 同梱の `corepack enable pnpm` で導入可。pnpm 無しで npm 運用する手順はステップ 1 参照)
...
# pnpm が無ければ Node.js 20+ 同梱の corepack で有効化(推奨)
corepack enable pnpm

pnpm install
```

> **pnpm を入れたくない場合**: 本 README のコマンドは `pnpm <script>` / `pnpm wrangler ...` 形式ですが、`pnpm install` の代わりに `npm install` でも動きます。その場合は各コマンドを次のように読み替えてください。
> - `pnpm run setup:google` → `npx tsx scripts/setup-google.ts`(env は同じく前置き)
> - `pnpm wrangler <...>` → `node_modules/.bin/wrangler <...>` または `npx wrangler <...>`
> - `pnpm deploy` → `node_modules/.bin/wrangler deploy`
>
> ⚠ `npm install` は `package-lock.json` を生成します(このリポジトリは `pnpm-lock.yaml` を採用)。混在を避けたい場合は commit しないでください。
