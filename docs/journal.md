...
## 2026-07-02 / GoogleHealthProvider 実装(移行本体)

Opus 4.8 の移行調査(スコープ対応表、エンドポイント形状、dataType マッピング、refresh token 7 日問題)をもとに、`src/providers/google-health/` に `HealthProvider` の Google Health API v4 実装を追加した。ツール層・キャッシュ層・guard は無変更(Provider 抽象が設計どおり効いた)。

### 変更点

- **`src/providers/google-health/`**(新規): `oauth.ts` / `client.ts` / `datapoints.ts` + Fitbit と同じドメイン分割(profile / device / activity / heart / sleep / body / nutrition / metrics / index)
- **`src/server.ts`**: `HEALTH_PROVIDER` env(`google` | `fitbit`)で provider を選択。**未設定時は fitbit にフォールバック** — 既存デプロイが pull + deploy しただけで壊れないため。`wrangler.toml.example` は `"google"` を設定済み
- **`src/lib/errors.ts`**: `GoogleHealthAuthError` / `GoogleHealthApiError` / `GoogleHealthRateLimitError` を追加。auth エラーのヒントに「Testing だと refresh token 7 日失効 → In production に publish して setup:google 再実行」を明記
- **`scripts/setup-google.ts`**(新規、`pnpm run setup:google`): Fitbit 版と同じワンショット CLI。差分は authorize URL(`accounts.google.com/o/oauth2/v2/auth`)+ `access_type=offline` + `prompt=consent`、token 交換が Basic ではなく form body、callback `/google/callback`。実行前に In production 昇格を促す警告を表示
- **`src/providers/types.ts`**: `SkinTempDay.value.absolute` を追加(Google は絶対 ℃、Fitbit は相対値 — 意味論が違うので別フィールド)
- テスト: `test/providers/google-health/oauth.test.ts` に 10 ケース(refresh_token 省略時に旧値を温存、form body に client_id/client_secret、Basic ヘッダ無し、等)。全 64 テスト緑

### Google 側の構造マッピング(実装の要点)

- 全メトリクスが単一リソース `POST/PATCH /v4/users/me/dataTypes/{dataType}/dataPoints(:list|:dailyRollUp|:batchDelete)` に集約。`datapoints.ts` に共通ヘルパー(`listDataPoints` はページネーション付き、`dailyRollUp`、`patchDataPoints`、`batchDeleteDataPoints`)を置き、ドメイン側は整形だけ
- Fitbit 操作 → Google メソッド: daily summary → `dailyRollUp`(複数 dataType を並列)、time series → `dailyRollUp`、intraday/ログ一覧 → `:list`、書き込み → `PATCH`、削除 → `:batchDelete`。プロフィール系は `users/me/profile` + `users/me/settings` + `users/me/identity`、デバイスは `users/me/pairedDevices`
- **KV `TOKENS` は 4 キーそのまま流用**(user_id は `'me'`)。Google の refresh 応答は `refresh_token` を省略することがあるので、その場合は旧値を KV に書き戻す
- **Intraday 心拍**: Google はネイティブ ~5s サンプルのみ。detailLevel(1min/5min/15min)は Worker 内の窓平均で再現、`1sec` はネイティブ粒度そのまま
- **睡眠**: `dateOfSleep`(起床日)は endTime の JST 日付から導出。取得窓は前日正午から開いて re-filter。`log_sleep` は「date = 起床日の朝」契約に合わせ、startTime が 18:00 以降なら前日開始とみなすヒューリスティック
- **食事**: `nutrition-log` への PATCH。meal slot ごとの既定 JST 時刻(朝 08:00 / 昼 12:30 / 夜 19:00 …)でタイムスタンプ化。栄養素キーは Fitbit 実測時代の `protein/totalFat/totalCarbohydrate/dietaryFiber/sodium/sugars` を第一候補として送信。水分は `hydration` dataType
- **`get_activity_timeseries`**: steps / distance / calories / activityCalories のみマッピング。floors・elevation・minutes 系は対応 dataType が無いので明示エラー(黙って間違った数字を返さない)

### 割り切り(重要): レスポンス整形は「寛容パース」

developers.google.com がこの実行環境から到達不可(proxy のネットワークポリシーで 403)だったため、**フィールド名レベルの正確な仕様は未検証**。API 自体も "actively evolving"(5/26 にも breaking change)なので、次の方針を採った:

- レスポンス schema は `z.record(z.string(), z.unknown())` ベースで受け、`pickNumber(record, ['steps', 'count', 'total', ...])` のように**候補キーを順に探索**して整形する
- 候補が全部外れてもツール呼び出しは失敗せず、該当フィールドが `undefined` に落ちるだけ。生 body のプレビューは schema 失敗時のエラーに含まれる(Fitbit client と同じ診断設計)
- data point id が数値でなかった場合、read はFNV ハッシュ代替で表示を続ける(console.log に警告)。この場合 `delete_*` は解決できない — 実データで要確認

### 実機検証チェックリスト(デプロイ後にやること)

0. Fitbit アカウントの Google 統合状態を確認(未統合だとデータが引けない。7/15 削除期限は今日時点で過ぎている — データ残存も確認)
1. GCP: API 有効化 → スコープ追加(Data Access で Save)→ OAuth client(Web application、redirect `http://127.0.0.1:8787/google/callback`)→ **In production 昇格** → `pnpm run setup:google` → KV 投入 → `HEALTH_PROVIDER = "google"` で deploy
2. `get_profile` / `list_devices` … 認証疎通 + profile/settings/identity のフィールド名が候補キーに乗っているか
3. `get_daily_summary` / `get_activity_timeseries(steps)` … `dailyRollUp` のバケット形状(`date` / `startTime` キー)と値キー
4. `get_sleep` / `get_heart_rate_intraday` … `:list` のページネーション、stage 配列のキー(`stages`?)、~5s サンプルの `bpm` キー
5. `log_water` → `get_food_log` → `delete_water_log` … PATCH の echo に dataPoints が返るか、**id が数値か**(delete の成立条件)
6. `log_food` / `log_meal_photo` … 栄養素キー(`totalCarbohydrate` 等)が Google 側でも保存されるか。外れていたら `scripts/diagnose-food-log.ts` 方式で実測して候補キーを修正
7. 皮膚温・HRV・SpO2 … 値キーの実測(特に皮膚温の絶対値/相対値)

キー名が外れていた場合の修正はすべて `src/providers/google-health/` の候補キー配列の先頭に実測値を足すだけで済む構造にしてある。
